...
func (a *AccountDB) PutConversation(ctx context.Context, c domain.Conversation) error {
	...
	`, c.TalkerID, c.AccountID, string(c.Kind), c.DisplayName, c.Avatar, timeUnixMilli(c.LastMsgTime), c.MsgCount)
	return err
}
...
func (a *AccountDB) PutMessage(ctx context.Context, m domain.Message) error {
	...
	`, m.TalkerID, m.MsgID, m.AccountID, m.MsgSeq, m.MsgType, boolInt(m.IsSend), timeUnixMilli(m.CreateTime), m.Text, m.XML, extra)
	return err
}
...
func (a *AccountDB) ListMessages(ctx context.Context, talkerID string, before time.Time, limit int) ([]domain.Message, error) {
	if talkerID == "" {
	...
	q := messageSelect + `
		WHERE talker_id = ?
		ORDER BY create_time DESC, msg_seq DESC
		LIMIT ?`
	args := []any{talkerID, limit}
	if !before.IsZero() {
		q = messageSelect + `
			WHERE talker_id = ? AND create_time < ?
			ORDER BY create_time DESC, msg_seq DESC
			LIMIT ?`
		args = []any{talkerID, timeUnixMilli(before), limit}
	}
...
func (a *AccountDB) PutMedia(ctx context.Context, m domain.MediaObject) error {
	...
	`, m.MediaID, m.AccountID, string(m.Kind), m.SHA256, m.Path, m.Size, boolInt(m.Available))
	return err
}
...
func (a *AccountDB) PutCursor(ctx context.Context, c domain.BackupCursor) error {
	...
	`, c.TalkerID, c.AccountID, timeUnixMilli(c.LastEndTime), c.SegmentMeta, c.Received, c.Total)
	return err
}
...
func scanMessage(sc rowScanner) (domain.Message, error) {
	...
	m.IsSend = isSend != 0
	m.CreateTime = unixMilliTime(created)
